...
con.on("open", function () {
  console.log("mogoDBAdmin connection established  successfully..!");
});
...
router.get(
  ...
  async (req, res) => {
    ...
    if (admin) res.redirect("/admin/page");
    else res.send("olaola");
  }
);
...
router.get("/StudentRes", bodyPar, (req, res) => {
  ...
  wb.xlsx.readFile("./public/ExcelData/A.xlsx").then(() => {
    ...
    for (let i = 2; i <= rowCount; i++) {
      obj[`${ws.getRow(i).getCell(1).value}`] = ws
        .getRow(i)
        .getCell(colCount).value;
    }
    res.send(obj);
  });
});
