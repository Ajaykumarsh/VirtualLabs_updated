...
const QuizSubmitorNot=(req,res,next)=>{
  ...
  MongoClient.connect(process.env.MONGODB_URI||'mongodb://localhost:27017/Studentdbex',async(err,db)=>{
    if(err) throw err;
...
router.post("/osi", urlencodedParser,  (req, res) => {
  ...
  MongoClient.connect(process.env.MONGODB_URI || 'mongodb://localhost/Studentdbex', async (err, db)=> {
    if (err) throw err;
